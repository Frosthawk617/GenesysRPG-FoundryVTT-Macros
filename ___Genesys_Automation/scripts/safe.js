async function main(){
            ...
            nextDialogue("Defense",page2Final,(html) => {
                ...
                nextDialogue("Skills",page3Final, (html)=>{
                    ...
                    nextDialogue("Talents",page4Final, (html)=>{
                        updateActor(selectedChar, selectedDefArray, selectedSkills);
                    })
                })
            })
   ...
   async function nextDialogue(title,content, callback) {
    ...
    async function buildPage4() {
        ...
        var page = '';
        var i = 0;
        for (var element of talents) {
            var id = element.id;
            const doc = await pack.getDocument(id);
            const data = await game.items.fromCompendium(doc);
          var pageAdd = `<tr><td><input type="checkbox" name="" id="select-`+i+`" class="skills"></td><td>`+data.name+`</td><td>`+data.description+`</td></tr>`;
           i++;
         page = page + pageAdd;
    }
    var pageString = page.toString();
    ...
    async function updateActor(selectedChar, selectedDefArray, selectedSkills, type){
        // Attributes------------------------------------
    ...
    target.update({'data.attributes.Wounds.value': wound});
    ...
    const id = pack.index.getName("Basic Military Training")._id;
    const doc = await pack.getDocument(id);
